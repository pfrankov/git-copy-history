...
module.exports = function(command, source, _options) {
    ...
    function getHistory() {
        ...
        if (!author) {
            author = execSync(`cd ${source} && git config user.name`, { encoding: 'utf8' }).replace(/\n/, '');
        }

        return execSync(`cd ${source} && git log --pretty="%H|%ad" --date=format:'%Y-%m-%d %H:%M:%S' --author="${author}"`, { encoding: 'utf8'});
    }
    ...
    function getCurrentDirHistory() {
        ...
        return execSync(`git log --pretty="%s|%ad" --date=format:'%Y-%m-%d %H:%M:%S' --author="${author}"`, { encoding: 'utf8' });
    }

    function findMissedCommits() {
        const historyArray = getHistory().split('\n').filter(x => x);
        let currentDirHistoryArray = getCurrentDirHistory().split('\n').filter(x => x);
    ...
    ({
        async from () {
            ...
            try {
                ...
                console.warn(chalk.bold.green(`No commits in current directory. Continue`));
                lines = getHistory().split('\n').filter(x => x);
            }
